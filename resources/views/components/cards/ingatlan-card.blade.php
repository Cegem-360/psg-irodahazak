<div
    ...
    <div class="{{ isset($small) && $small ? 'p-3 pl-4' : 'p-6 pl-8' }}">
        ...
        <p class="{{ isset($small) && $small ? 'text-gray-700 text-xs min-h-16' : 'text-gray-700 min-h-24' }}">
            {!! $description !!}</p>
        <a href="{{ $link ?? '#' }}"
            class="inline-block {{ isset($small) && $small ? 'mb-2 px-3 py-1 text-sm' : 'mb-4 px-6 py-2' }} bg-primary/70 text-white rounded group-hover:bg-primary/90 transition-colors duration-300 ease-in-out after:absolute after:inset-0 after:cursor-pointer">
            További részletek
        </a>
    </div>
</div>
